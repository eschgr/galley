...
Why the current code does not already do this: **Node has no built-in, cross-platform advisory file lock.** `flock`/`fcntl` are not in Node core, and the npm packages that add them are **native addons**, which the main PRD section 8 rules out (no Rust / no C/C++ build tools). The original channel therefore used a pure-JS PID probe plus a `.ping`→`.pong` file handshake; the handshake is answered on the main-process event loop, which is exactly what a native modal blocks ([#56](https://github.com/eschgr/mdtool/issues/56)).

Three candidate pure-JS mechanisms, to be settled by a **spike** (section 10, Phase 2) on both Windows and macOS against the real modal-block repro:

1. **Held exclusive file handle (`O_EXLOCK`).** Open a lock file with an OS exclusive-lock flag and keep the handle open: a second opener fails while it is held, and the OS releases it automatically on process death. The most direct expression of "a lock", needs no ack, immune to the modal block. Cross-platform nuance the spike must confirm: on **Windows**, file locking is mandatory — holding a file open with an exclusive share mode genuinely blocks other writers and is released on death — but **Node's `fs` opens with a permissive share mode by default and does not expose share-mode control**; on **macOS/POSIX**, plain "holding open" is only advisory. The bridge is the `O_EXLOCK` open flag, which libuv maps to an exclusive share mode on Windows and an exclusive advisory lock on macOS — *if* Node reliably exposes and honors `fs.constants.O_EXLOCK` on both, this is one pure-JS call that works cross-platform. Whether Windows honors it is the spike's first question.
2. **Start-time-qualified PID liveness (no ack).** Trust `process.kill(pid, 0)` — answered by the OS, so it stays true while the owner's main thread is stuck in a modal — and defeat PID reuse by matching the owner process's **OS start-time** to a value recorded at claim (same PID + different start-time ⇒ recycled ⇒ dead). Start-time is readable without a native addon (`wmic`/PowerShell on Windows, `ps -o lstart` on macOS). Removes the ack that causes [#56](https://github.com/eschgr/mdtool/issues/56) while keeping reuse-safety; no special file handle.
3. **Off-main-thread holder.** A small Electron `utilityProcess` holds the liveness signal on its own event loop (a modal on the main thread cannot stall it) and is reaped by the OS on app death. More robust in principle, more moving parts.

Recommendation: spike the held handle (#1) first — the most direct realization of an OS-maintained lock; fall to start-time (#2) if `O_EXLOCK` is not honored cross-platform, and to the off-main-thread holder (#3) only if neither suffices. Either way the caller contract is unchanged.
