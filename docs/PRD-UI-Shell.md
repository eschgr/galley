...
- **R13. Open the PDF after export.** As soon as an exported PDF is written (R8), Galley opens it in the OS default PDF viewer, so the result is visible without hunting through the file system. Opening is best-effort and strictly ordered after a successful write: a viewer that cannot be launched surfaces a non-fatal notice (the PDF is already on disk), and a write that fails opens nothing. Uses the built-in Electron shell handler — no new dependency (R11).
