...
- **R12b. Source-editor focus on activation.** While the source pane is shown, keyboard focus lands in the **active tab's source editor** whenever it becomes the surface to type into — switching to a tab, opening a file, or revealing the editor with **Show Source** — so the user can start typing without a click. Every tab stays mounted, so its editor keeps its own cursor, selection, and scroll position (the tab/instance model is [`PRD-UI-Shell.md`](PRD-UI-Shell.md)'s concern); this restores only the keyboard focus a hidden (`display:none`) editor loses, and does **not** move either pane's scroll position (R9, R9a). When the source pane is hidden (reading mode) focus is never pulled into the hidden editor. **Exception:** following a clicked cross-file link (`other.md#section`, R4) is a reading action, so focus stays in the preview at the revealed heading. *(Interacts with R12/R12a: once the editor holds focus, `Cmd/Ctrl+F` opens the editor's own find; the preview find stays the default while the source pane is hidden or the preview is focused.)*
...
- **R22. Spell-checking & the editor context menu.** The source editor enables the browser's native spell-checker: misspelled words get the usual red underline. **Right-clicking in the editor opens its context menu** — the standard edit actions (Cut / Copy / Paste / Select All) always, and, when the click lands on a misspelled word, that word's **spelling suggestions** and **Add to Dictionary** (persistent) prepended above them. *(The squiggles are native; the menu itself is built by a main-process context-menu handler — the browser leaves building it to the app — so the spell items are one conditional section of the editor's context menu, which is also where any future editor entries live. Prose-oriented — text inside fenced code blocks is checked too, an accepted trade-off vs. a much heavier Markdown-aware checker. Only a permanent add-to-dictionary is offered, not a session-only "ignore" — the native checker doesn't support one. Squiggles cover the on-screen text; the editor virtualizes off-screen lines, so misspellings on lines not yet edited or scrolled into view may not be flagged — whole-document coverage is tracked in #132.)*
