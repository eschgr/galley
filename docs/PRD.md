...
- **R4.** Clicking a link **in the preview** opens it in the **system default browser**, never inside the app's own window. *(Also a safety requirement — prevents the renderer from navigating away or spawning an in-app browser window.)* **Exception: in-page anchor links** (`#heading`) scroll the preview to that heading instead of leaving the app — headings are given GitHub-style id slugs so the LLM's cross-reference links work.
